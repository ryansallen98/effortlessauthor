...
import EditorMock from "../../components/mocks/EditorMock.astro";
...
<Figure
  caption="The editor screen and its five regions."
  min={780}
  max={920}
  legend={[
    { n: 1, label: "App bar — save, preview, settings, and the KDP-aware Export EPUB action" },
    { n: 2, label: "File tree — your manuscript as sections and files; add, rename, and drag to reorder" },
    { n: 3, label: "Markdown editor — write in plain Markdown with formatting shortcuts" },
    { n: 4, label: "Live preview — a rendered view that updates as you type" },
    { n: 5, label: "Status bar — file path, word count, and a KDP-ready check" },
  ]}
>
  <EditorMock annotate />
</Figure>
