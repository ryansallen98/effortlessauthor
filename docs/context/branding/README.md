...
- **Markdown content:** policy pages and resource guides are editable Markdown in `src/content/legal/**` and `src/content/guides/**`, typed by `src/content.config.ts` (Astro content collections, glob loader) and rendered through the dynamic routes with the `.prose` styles. To add/edit a policy or guide, just edit the Markdown + frontmatter.
...
- **Forms:** the early-access/demo forms are visually complete but not yet wired to a backend (`onsubmit` is a no-op). Connect a form endpoint (e.g. a serverless function or form service) before launch.
